...
controller.getMovies = (req, res) => {
  ...
  connection.query(`SELECT title,director, main_actor, release_date, category, status
  FROM movies LEFT JOIN user_movie ON movies.movie_id = user_movie.movie_id 
...
controller.getMovie = (req, res) => {
  ...
  connection.query(`SELECT title, director, main_actor, release_date, category, status FROM movies
